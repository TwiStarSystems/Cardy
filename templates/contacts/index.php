...
<div class="page-header">
  ...
  <div class="flex gap-sm flex-wrap">
    <?php /* Address Book Switcher */ ?>
    <div class="dropdown" id="ab-switcher-dropdown">
      <button type="button" class="btn btn-secondary" id="ab-switcher-btn" title="Switch address book">
        ...
        <?= $_ctrl->e($activeBook['displayname'] ?? 'Address Book') ?>
      ...
      <div class="dropdown-menu" id="ab-switcher-menu" style="min-width:260px">
        ...
        <?php foreach ($allAddressBooks as $book): ?>
        ...
        <div class="dropdown-divider"></div>
        <?php /* Create new */ ?>
        <button type="button" class="dropdown-item" id="ab-create-toggle" style="display:flex;align-items:center;gap:6px">
        ...
        <div id="ab-create-form" style="display:none;padding:8px 12px">
          <form method="POST" action="/contacts/addressbooks">
            <input type="hidden" name="_csrf" value="<?= $_ctrl->e($csrf) ?>">
            <input type="text" name="displayname" class="form-control" placeholder="e.g. Work Contacts" required
        ...
        <div id="ab-rename-form" style="display:none;padding:8px 12px">
          <form method="POST" action="/contacts/addressbooks/<?= (int)($activeAbId ?? 0) ?>/rename">
            <input type="hidden" name="_csrf" value="<?= $_ctrl->e($csrf) ?>">
            <input type="text" name="displayname" class="form-control"
                   value="<?= $_ctrl->e($activeBook['displayname'] ?? '') ?>"
    ...
    <div class="dropdown" id="contacts-actions-dropdown">
      ...
      <div class="dropdown-menu" id="contacts-actions-menu">
        ...
        <form method="POST" action="/contacts/addressbooks/<?= (int)($activeAbId ?? 0) ?>/delete" style="display:contents">
          <input type="hidden" name="_csrf" value="<?= $_ctrl->e($csrf) ?>">
          <?php $abDelMsg = 'Delete address book "' . ($activeBook['displayname'] ?? 'this address book') . '"? All ' . (int)($activeBook['card_count'] ?? 0) . ' contacts in it will be permanently deleted.'; ?>
          <button type="submit" class="dropdown-item text-danger"
                  onclick="return confirm(<?= htmlspecialchars(json_encode($abDelMsg), ENT_QUOTES) ?>)">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" style="width:15px;height:15px;flex-shrink:0;color:var(--color-danger,#e53e3e)">
...
(function () {
  ...
  initDropdown('ab-switcher-btn', 'ab-switcher-menu');

  // ---------- AB switcher inline forms ----------
  function togglePanel(toggleBtnId, panelId) {
